...
* Handle empty ``GetActions`` responses by clearing ``_observation_pending``.
* Add ``openarm_follower`` to imports and ``SUPPORTED_ROBOTS``.
"""
...
from lerobot.async_inference import constants as _constants
...
# ---------------------------------------------------------------------------
# 1. Patch SUPPORTED_ROBOTS
# ---------------------------------------------------------------------------
if "openarm_follower" not in _constants.SUPPORTED_ROBOTS:
    _constants.SUPPORTED_ROBOTS.append("openarm_follower")

# ---------------------------------------------------------------------------
# 2. Patch RobotClient.__init__ — add _observation_pending event
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 3. Patch _aggregate_action_queues
# ---------------------------------------------------------------------------
    ...
    while self.running:
        try:
            ...
            if len(actions_chunk.data) == 0:
                # Server returned Empty: either obs was filtered out or
                # obs_queue_timeout expired.  Unblock the control loop so it
                # can send a fresh observation on the next tick.
...
# ---------------------------------------------------------------------------
# 5. Patch control_loop_action (verbose default only)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 6. Patch _ready_to_send_observation
# ---------------------------------------------------------------------------
...
def _patched_ready_to_send_observation(self):
    ...
    # Always allow sending if queue is empty (must-go recovery path)
    if queue_empty:
        self._observation_pending.clear()
        return True
...
# ---------------------------------------------------------------------------
# 7. Patch control_loop_observation
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 8. Patch control_loop (verbose default only)
# ---------------------------------------------------------------------------
